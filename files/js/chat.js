document.addEventListener("DOMContentLoaded", function () {
   ...
   document.querySelectorAll('.chat__wrap-message-headerWrapMenu').forEach(menu => {
      ...
      leaveFeedbackItem.addEventListener('click', () => {
         ...
         reviewBlock.innerHTML = `<div class="chat__leave-a-reviewHead">
                     ...
                     <div class="rate-this-article__wrapStarsRating">
                        ${[1,2,3,4,5].map(num => `
                           <label>
                              ...
                              <img class="no-active" src="images/svg/chat-star.svg" alt="">
                              <img class="active" src="images/svg/chat-star-active.svg" alt="">
                           </label>
                        `).join('')}
                     </div>
                  </div>
            ...
            <button type="button" class="chat__leave-a-reviewButton" disabled>Опублікувати</button>`;
   ...
   /*function initReviewBlock(reviewBlock) {
      ...
      function handleFileInput(event) {
         ...
         reader.onload = function (e) {
            ...
            removeBtn.innerHTML = '<svg width="7" height="7" viewBox="0 0 7 7" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M6.44625 0.15125C6.25125 -0.0437499 5.93625 -0.0437499 5.74125 0.15125L3.29625 2.59125L0.85125 0.14625C0.65625 -0.04875 0.34125 -0.04875 0.14625 0.14625C-0.04875 0.34125 -0.04875 0.65625 0.14625 0.85125L2.59125 3.29625L0.14625 5.74125C-0.04875 5.93625 -0.04875 6.25125 0.14625 6.44625C0.34125 6.64125 0.65625 6.64125 0.85125 6.44625L3.29625 4.00125L5.74125 6.44625C5.93625 6.64125 6.25125 6.64125 6.44625 6.44625C6.64125 6.25125 6.64125 5.93625 6.44625 5.74125L4.00125 3.29625L6.44625 0.85125C6.63625 0.66125 6.63625 0.34125 6.44625 0.15125V0.15125Z" fill="#20201E"/></svg>';
   ...
   function initReviewBlock(reviewBlock) {
      ...
      if (reviewBlock.dataset.initialized) {
         return;
      }
      reviewBlock.dataset.initialized = 'true';
      ...
      const addReviewBtn = reviewBlock.querySelector('.chat__leave-a-reviewButton');
      if (addReviewBtn) {
         addReviewBtn.addEventListener('click', () => {
            submitReview(reviewBlock);
         });
      }
   }
   ...
   // По клику на кнопку Опублікувати создается сообщение с содержимым отзыва
   function submitReview(reviewBlock) {
      const parentMessage = reviewBlock.closest('.chat__wrap-message');

      if (!parentMessage) return;

      const messageContainer = parentMessage.querySelector('.chat__wrap-message-message');

      if (!messageContainer) return;

      const reviewText = reviewBlock.querySelector('textarea')?.value.trim() || '';

      const rating = reviewBlock.querySelector('input[name="review-rating"]:checked')?.value || 0;
      ...
      reviewMessage.classList.add('user-sms', 'author', 'review-message');

      let imagesHtml = '';

      reviewBlock.querySelectorAll('.image-preview img').forEach(img => {
         imagesHtml += `<div class="review-image">
            <img src="${img.src}" alt="">
         </div>`;
      });
      ...
      for (let i = 1; i <= 5; i++) {
         const starIcon = i <= rating ? '/images/svg/chat-star-in-added-review-active.svg' : '/images/svg/chat-star-in-added-review.svg';
         starsHtml += `<img src="${starIcon}" alt="" class="review-star-icon" width="20" height="20">`;
      }

      reviewMessage.innerHTML = `<h4>Залишено новий відгук!</h4><div class="review-rating"><p>Оцінка:</p>${starsHtml}</div>

      ${reviewText ? `<p class="user-sms-text">${reviewText}</p>` : ''}
      ${imagesHtml ? `<div class="review-images">${imagesHtml}</div>` : ''}

      <span class="user-sms-time">${time}</span>`;

      // Уведомление
      const notification = document.createElement('div');

      notification.classList.add('chat__notification', 'sent-for-review');

      notification.innerHTML = `<p>Ваш відгук надіслано на <br>перевірку</p>`;

      messageContainer.appendChild(reviewMessage);
